...
- 本应用启用了 ingress，应用页的「打开 Web UI」与侧边栏面板会打开容器内 8080 的页面
- 注意：面板内依赖 HTTP API（1985）或 WebRTC（8000/UDP）的功能不可用（ingress 只代理单一端口），播放与 API 调用请直接使用 `http://<HA>:8080` / `http://<HA>:1985`
...
| 8080/tcp | HLS/HTTP-FLV/内置播放页 | 主机端口可在应用页面修改（ingress 固定走容器 8080） |
